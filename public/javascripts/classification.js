...
var chose_attribute = function(attrib, examples) {
    ...
    best.name = index_of_best_value(entropy); // TODO: has to become gain instead of plain entropy
...
var prism_offline = function(examples, attrib, def) {
    function prism_offline_lim(examples, attrib, def, lim) {
        if (lim<=0) return {
            "label": majority_value(examples),
            "type": "leaf",
            "subtrees": [],
        ...
        if (examples.length == 0) return {"label": def, "type": "leaf", "subtrees": []};
        ...
        else if (attrib.length == 0) return {
            "label": majority_value(examples),
            "type": "leaf",
            "subtrees": [],
            "note": "majority"
            ...
            for (var i in best.values) {
                ...
                var subtree = {
                    "label": best.values[i],
                    "subtrees": [prism_offline_lim(examples_i, attrib, m, lim-1)],
                    "type": "option"
                };
                tree.subtrees.push(subtree);
            }
    ...
    return prism_offline_lim(examples, attrib, def, attrib.length*2);
};